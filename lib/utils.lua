...
function easing.loop2(ease)
    return function(t, tMax, start, delta)
        if t / tMax < 0.5 then
            return ease(t, tMax * 0.5, start, delta)
        else
            return ease(t, tMax * 0.5, start, delta)
        end
    end
...
function easing.swing3(ease)
    return function(t, tMax, start, delta)
        if t / tMax < 0.33 then
            return ease(t, tMax * 0.33, start, delta)
        elseif t / tMax < 0.66 then
            return ease(t - tMax * 0.33, tMax * 0.33, start + delta, -delta)
        else
            return ease(t - tMax * 0.66, tMax * 0.66, start, delta)
        end
    end
end
